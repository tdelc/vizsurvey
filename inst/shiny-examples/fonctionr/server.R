...
server <- function(input, output, session) {
      ...
      local({
        ...
        if (my_i %in% values_ini$config$vd){
          ...
          output[[plot_evol_name]] <- renderPlot({
            plot <- shiny_distrib_discrete(
              db = df_zone_compa, quali_var = values_ini$config$vt,
        ...
        if (my_i %in% values_ini$config$vc){
          ...
          output[[plot_evol_name]] <- renderPlot({
            plot <- shiny_distrib_continuous(
              ...
              filter_exp=TRUE,group=values_ini$config$vt,type="median")
            plot$graph
          })
        }
      })
  ...
  output$itw_distrib_spe <- renderPlot({
    ...
    filtre_itw <- paste0(values_ini$config$id_itw, " == '",values_itw$id_itw,"'")
    ...
    if (!fl_discrete & values_itw$variable %in% values_ini$config$vc){
    ...
    } else if (fl_discrete | values_itw$variable %in% values_ini$config$vd){
  ...
  output$itw_distrib_glo <- renderPlot({
    ...
    filtre_ne_itw <- paste0(values_ini$config$id_itw," != '",values_itw$id_itw,"'")
  ...
  observeEvent(input$config_modalities_itw,{
    ...
    syn_distrib <- df_sub() %>%
      ...
      mutate(N = sum(n),prop = n/N) %>%
      ungroup() %>%
      complete(!!sym(values_ini$config$id_itw),!!sym(values_itw$variable),

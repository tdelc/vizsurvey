...
Pour lancer l'interface à partir d'un objet R, il suffit de
...
Si vous avez lancé les packages nécessaires, une interface {shiny} doit se lancer dans votre navigateur
...
, et afficher cette carte de chaleur. Celle-ci est le cœur du fonctionnement de {vizsurvey}, car elle permet de naviguer de manière interactive à travers les situations détectées comme anormales.
...
## Analyser une variable catégorielle
...
Pour essayer de comprendre les différences, nous avons inclu un algorithme qui calcule les dix corrélations les plus hautes dans la base de données, pour ensuite projeter les corrélations effectives pour l'enquêteur-rice et celle du reste de la base de données. Il s'agit uniquement de corrélations de pearson en transformant au préalable les variables catégorielles en facteur.
...
Lorsqu'on clique sur une variable numérique, la fonction de densité s'affiche alors, toujours avec le point de comparaison entre l'enquêteur-rice, et le reste de la base de données. Dans notre cas Voici un exemple pour la variable d'âge pour l'enquêtrice Carinthia-5 :

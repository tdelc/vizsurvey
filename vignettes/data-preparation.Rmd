...
subtitle: "Vignette 2 — Préparer les données"
...
Cette vignette se concentre sur la préparation des données pour utiliser {vizsurvey} avec l'appel `runVizsurvey_from_folder`. L'objectif ici est de pouvoir utiliser {vizsurvey} de manière régulière, dans un équipe devant gérer plusieurs enquêtes.

Dans cette situation, devoir lancer l'interface pour une seule base de données est une limitation peu pratique. De même, faire les calculs d'écarts sont gourmands en calculs, notamment si on multiplie les domaines et les groupes.

{vizsurvey} contient plusieurs fonctions permettant de préparer les données, et de préparer les données en une seule étape. Cette vignette présente cela pas à pas.

# Préparer vos données

## Structure simple : 1 seul fichier

Le fichier de votre base de données doivent pouvoir être lus par la fonction `data.table::fread`. Cette fonction détecte automatiquement le format de vos données pour les formats standards (csv/tsv). Vous devez placer le fichier dans un répertoire où il est le seul fichier du même type.

### Fichier de configuration de l'enquête

{vizsurvey} requiert qu'un fichier de configuration `config.txt` soit placé au même emplacement que votre base de données. Voici les éléments qu'il doit comporter :
...
-   `name_survey` : nom de l'enquête, utile pour distinguer les fichiers

-   `vars_discretes` : liste des variables dont on veut forcer le format catégoriel

-   `vars_continous` : liste des variables dont on veut forcer le format continus

-   var_domain : variable de domaine, pour séparer chaque calcul selon cette variable (exemple : année)

-   var_group : variable de groupe, pour séparer les calculs également. À savoir que les calculs sont également effectué sans distinction des groupes (exemple : province)

-   var_itw : variable identifiant l'enquêteur-rice

Vous pouvez créer vous même le fichier, ou faire appel à la fonction dédiée `create_config`. Voici la création du fichier config.txt pour les fichiers ménages de l'enquête SILC, avec l'année comme variable de domaine (`HB010`) et la province comme variable de groupe (`HB020`) :
...
Les variables discrètes et continues ne doivent être forcées que si vous constatez une mauvaise classification de la part de la fonction dédiée `classify_variables` qui préparera ensuite le fichier. Cette fonction détermine le type de variable en fonction du format de la colonne, mais également selon le nombre de modalités. Le nombre de modalités maximal pour classer une variable comme catégorielle est modifiable avec l'argument `threhold` et est fixé par défaut à 15.
...
{vizsurvey} permet de calculer en avancer toutes les statistiques d'écarts de votre enquête. La fonction `prepa_survey` part de votre répertoire contenant le fichier de données et le fichier config.txt pour générer :

-   Par domaine, Les statistiques de chaque variable ;
...
La fonction `prepa_survey` peut inclure un pattern pour la base de données. Par défaut, seuls les fichiers csv sont inclus. La fonction génère un fichier global.rda dans le même répertoire que les données. C'est ce fichier qui sera utilisé par l'interface interactive.
...
Il suffit d'appeler la fonction runVizsurvey_from_folder avec le nom du répertoire, et l'argument

```{r}
runVizsurvey_from_folder("inst/extdata/SILC/HFILE",
                         is_double_folder = F)
```

## 1. Structurer les fichiers d'entrée

Les données du package sont stockées avec cette arborescence :

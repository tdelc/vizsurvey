...
Ces choix méthodologiques ne sont pas figés : ils ont été progressivement affinés au sein de Statbel, sur base des premiers résultats produits par les tableaux de contrôle.\
Les retours d’expérience des coordinateur-rices d’enquête, les échanges entre départements, ainsi que plusieurs références issues de la littérature sur la qualité des enquêtes ont guidé l’évolution des fonctions. Ainsi, `{vizsurvey}` reflète une démarche collective et empirique, fondée sur la confrontation directe aux données plutôt que sur des prescriptions théoriques.

La publication de ce package vise à partager ces choix, non à imposer une solution unique. En rendant publiques les formules, les fonctions et la logique de calcul, nous souhaitons favoriser la transparence, encourager la réutilisation et stimuler les contributions d’autres équipes confrontées à des problématiques similaires. Notre ambition est de construire, avec d’autres instituts de statistique et organismes de sondage, une base commune d’outils et de pratiques pour le monitoring des enquêtes, en intégrant les retours des enquêteur·rices, des superviseur·es et des méthodologues.
...
# Construction progressive et validation empirique

Le choix des indicateurs et des méthodes de calcul a été progressivement affiné au sein de Statbel, sur base des premiers résultats observés lors des contrôles de cohérence des enquêtes (SILC, ESS, ICT, etc.). Cette approche **empirique et collaborative** a permis d’identifier les combinaisons d’indicateurs les plus pertinentes pour...

Plusieurs **références méthodologiques et articles scientifiques** ont également inspiré les choix retenus, notamment dans le domaine du *data quality monitoring*, des *survey paradata*, et du *robust statistics*.

# Transparence et amélioration continue

Nous ne prétendons pas ici proposer une solution parfaite. L’objectif de la publication de `{vizsurvey}` est au contraire de **rendre ces formules transparentes**, pour encourager leur **réutilisation, discussion et amélioration**.

En rendant public le code source des fonctions et la logique de calcul :

-   nous permettons à d’autres instituts de **vérifier, adapter et améliorer** nos approches,

-   nous favorisons une **harmonisation méthodologique** entre pays et entre types d’enquêtes,

-   nous facilitons la **réplicabilité** des analyses et le **dialogue entre statisticiens**.

# Une collaboration ouverte

Le développement de `{vizsurvey}` s’inscrit dans une démarche de **coopération inter-institutionnelle**.\
Nous souhaitons travailler avec les **instituts nationaux de statistique**, les **organismes réalisant des enquêtes de grande ampleur**, et les **équipes de terrain** impliquant des **enquêteur·rices** et des **coordinateur·rices de collecte**.

L’objectif est de construire **collectivement une interface de contrôle efficace**, intégrant à la fois les besoins méthodologiques des analystes et les retours opérationnels des équipes de terrain.

...
Dans un processus statistique aligné sur le GSBPM, la qualité ne se joue
pas seulement en amont (échantillonnage, questionnaire, collecte) ou en
aval (analyse, diffusion). Entre la collecte et l’analyse se situe une
phase clé : le contrôle et le suivi du matériau collecté, pour repérer
...
D’une vague à l’autre, les questionnaires évoluent (libellés, routages,
...
- les variables présentant des ruptures qui méritent un second regard
  (mise en œuvre du routage, recodage, version du questionnaire,
  changement de mode de collecte, etc.).
...
obligatoire, le second facultatif, en cascade dans l’interface.
...
sein de chaque valeur du filtre.
...
Cette fonction accepte aussi trois fichiers transverses facultatifs,
décrits dans la vignette de préparation : un **dictionnaire** des
variables (`path_dict`), une **nomenclature** de libellés
(`path_nomen`), et un fichier d’**archive** partagé (`path_archive`).
...
- pour l’enquêteur·rice **Vienna-2** : l’âge est divisé par 10, le
  revenu équivalent par 100, et la modalité 5 de `pl030` est remplacée
  par des valeurs manquantes ;
- pour la vague **2020** : la variable `pb220a` est entièrement
  manquante, la modalité 4 de `hsize` disparaît, et le revenu équivalent
  est divisé par 100.
...
- le **chargement de la base** : le choix du dossier et de l’enquête (en
  mode `from_folder`) ;
- le **filtrage** : la vague et le filtre courants. La vague est
  obligatoire ; quand elle a deux niveaux, un second sélecteur apparaît
  en cascade sous le premier, facultatif (`All` par défaut). Le filtre
  fonctionne de la même façon, avec `All` pour ne pas filtrer ;
...
| Distance de chi² / écart de médiane minimum | 5 | écart standardisé à partir duquel un croisement enquêteur·rice × variable est signalé |

Il n’existe pas de seuil statistiquement fondé pour ce type d’analyse :
c’est à l’équipe de coordination de le régler par tâtonnement.
Baissez-le pour voir plus de cas, montez-le pour ne garder que les plus
francs.
...
variables analysées, enquêteur·rices, vagues), une carte d’identité
(nom, chemin, date de la dernière préparation, rôle des variables,
présence des timers) et le nombre d’entretiens par vague — découpé par
le second niveau de vague quand il existe.
...
× variable au-dessus des seuils. Chaque carte liste les trois cas les
...
indicateur du tableau *et* du score — utile quand un indicateur n’a pas
de sens pour l’enquête (le travail de nuit pour des entretiens sur
...
> responsabilité de l’équipe méthodologique.
...
lisent donc de la même façon, avec une seule distance de chi², au lieu
d’un mélange de chi² et d’écarts de médiane.
...
- la **distribution des modalités** : pour chaque modalité, la
  ...
  rouge sur la valeur de l’enquêteur·rice. Elle répond à une question
  que le graphique précédent ne traite pas : cet·te enquêteur·rice
  est-il·elle réellement isolé·e, ou beaucoup d’autres sont-ils au même
  niveau ?

![](images/enqueteur-detail-variable.png)

Sur les données d’exemple, le revenu de Vienna-2 (multiplié par 100)
saute aux yeux : sa densité est décalée d’un ordre de grandeur.
...
- les **variables numériques** qui ont bougé : même principe, avec la
  part de manquants et la médiane — préférée à la moyenne car moins
...
- la **table de chi²** : une ligne par couple enquêteur·rice × variable
  ...
  point d’entrée quand on ne sait pas par où commencer ;
- la **heatmap** : une ligne par enquêteur·rice, une colonne par
  ...
  affiche les valeurs sous-jacentes, le clic ouvre les distributions en
  bas de page ;
...
- la **synthèse par variable** : le même classement, vu du côté des
  ...
  problème de questionnaire (formulation ambiguë, routage) plutôt que
  d’un·e enquêteur·rice.

![](images/croise-heatmap.png)

La lecture de la heatmap tient en trois règles :
...
- une **ligne** régulièrement rouge suggère un comportement récurrent
  (erreur de procédure, incompréhension, routage mal appliqué, voire
  fraude) ;
- une **colonne** rouge désigne une variable fragile (libellé ambigu,
  routage, effet de sous-population) plutôt que les enquêteur·rices.
...
Enquêteur. Si un dictionnaire a été fourni, une carte affiche en plus
les entrées du dictionnaire pour la variable sélectionnée — pratique
pour vérifier la formulation exacte d’une question avant de conclure.
...
- **Archive** : l’historique des observations. Le bouton **Ajouter à
  l’historique** enregistre la date, l’utilisateur·rice, l’enquête, la
...
2.  **Partez** de l’onglet Synthèse : les trois points d’entrée donnent
    l’ampleur de ce qu’il y a à examiner.
3.  **Vérifiez** d’abord l’onglet Vague : une variable absente ou un
    ...
    ailleurs.
4.  **Parcourez** la heatmap de l’onglet Croisé : lignes rouges
    (enquêteur·rices), colonnes rouges (variables).
5.  **Cliquez** sur les cas marquants pour ouvrir les distributions, et
    comparez la forme, pas seulement l’écart.
6.  **Contrôlez** avec le filtre qu’il ne s’agit pas d’un effet de
    sous-population.
7.  **Consignez** chaque vérification dans l’Archive, y compris les
    fausses alertes.
